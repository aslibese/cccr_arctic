...
# reorder MMM data
MMM_aaf_reordered = MMM_aaf.isel(month=month_indices)
MMM_aaf_reordered = MMM_aaf_reordered.assign_coords(month=np.arange(1, 13))

# reorder model data
aaf_moving_reordered = aaf_moving.isel(month=month_indices)
aaf_moving_reordered = aaf_moving_reordered.assign_coords(month=np.arange(1, 13))
...
for i, model in enumerate(models):
    ax = axes[i]
    model_data = aaf_moving_reordered.sel(model=model)
...
ax = axes[23]
cf = ax.contourf(year_values, MMM_aaf_reordered['month'], MMM_aaf_reordered, levels=levels, cmap=cmap, extend='both')
# plot the dots
max_month_indices_MMM = MMM_aaf_reordered.argmax(dim='month') + 1  # months from 1 to 12
