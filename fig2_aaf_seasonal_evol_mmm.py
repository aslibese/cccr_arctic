...
year_values = MMM_aaf_reordered['year'].values
...
cf = ax.contourf(year_values, MMM_aaf_reordered['month'], MMM_aaf_reordered, levels=levels, cmap='Oranges', extend='both')

ax.set_yticks(np.arange(1, 13))
ax.set_yticklabels(month_names)

# compute the month of maximum value for each year
max_month_indices = MMM_aaf_reordered.argmax(dim='month') + 1  # months from 1 to 12
...
cbar.set_label('Arctic Amplification Factor', fontsize=14)
cbar.ax.tick_params(labelsize=12)

plt.tight_layout()

output_file = '/home/abese/arcticProject/cccr_arctic/figures/aaf_seasonal_evol_MMM.png'
